...
class PriceArchive:
  ...
  def get_price(self, ticker, date: datetime):
    ...
    price = self.prices.loc[(self.prices[TICKER] == ticker) & (self.prices[DATE] == int(date_str)), PRICE]
...
def calculate_index_coefficient(moex_archive, prices_archive, date):
  ...
  for orig_ticker in moex_list:
    ticker = orig_ticker
    if orig_ticker in TICKER_TO_ACTUAL_TICKER and TICKER_TO_ACTUAL_TICKER[orig_ticker] != "":
...
def main():
  ...
  MOEX_STATUSES_PATH = Path("/home/chereshnya/Me/Finance/MOEX_with_no_companies/data/moex_statuses")
  PRICES_PATH = Path("/home/chereshnya/Me/Finance/MOEX_with_no_companies/preprocessed_data/prices.csv")
